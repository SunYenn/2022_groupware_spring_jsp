	...
	<div class="tjcontainer">
        ...
        <div class="con_middle">
            ...
            <div class="content message">
            	<div>
            		<ul>
            			<li><a href="move_message_view?divs=receive">받은 쪽지함</a></li>
            			<li><a href="move_message_view?divs=send">보낸 쪽지함</a></li>
            			<li><a href="move_message_view?divs=trash" style="color: #398AB9">휴지통</a></li>
            		</ul>
            	</div>
                <div>
					...
					<table style="width: 900px; margin: auto;"> 
						...
						<c:if test="${view.size() != 0 }">
							<c:forEach var="meo" items="${view}">
									...
									<c:set var="title" value="${fn:replace(meo.title, '>', '&gt;') }"></c:set>
									<tr onclick="location.href='message_content_view?idx=${meo.idx}&currentPage=${MessageList.currentPage}&divs=trash'">
										<td>
											<div class="messageList">
													...
													<div class="messageDelete">
	 													<button onclick="
															...
															event.cancelBubble=true; 
	 														location.href='message_service?idx=${meo.idx}&currentPage=${currentPage}&mode=4'"
	 													>삭제</button>
	 												</div>
													<div class="restore">
	 													<button onclick="
															...
															event.cancelBubble=true; 
	 														location.href='message_service?idx=${meo.idx}&currentPage=${currentPage}&mode=3'"
	 													>복구</button>
	 												</div>
	 											</div>
												<div> ${meo.title} </div>
												<div>
													<span class="underText">
														보낸사람 : ${meo.transename}(${meo.transeempno}) | 받는사람: ${meo.receivename}(${meo.receiveempno}) |																										<c:if test="${today == sdf1date}">${sdf2date}</c:if>
