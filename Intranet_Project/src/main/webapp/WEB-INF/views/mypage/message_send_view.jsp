            ...
            <div class="content message">
            	<div>
            		<ul>
            			...
            			<li><a href="move_message_view?divs=send" style="color: #398AB9">보낸 쪽지함</a></li>
					...
					<table style="width: 900px; margin: auto;"> 
						...
						<c:if test="${view.size() != 0 }">
							<c:forEach var="meo" items="${view}">
									...
									<c:set var="title" value="${fn:replace(meo.title, '>', '&gt;') }"></c:set>
									<tr onclick="location.href='message_content_view?idx=${meo.idx}&currentPage=${MessageList.currentPage}&divs=send'">
										<td>
											<div class="messageList">
												<div style="float: right;">
													<c:if test="${meo.read == 'NO'}">
														<p class="messageread">읽지않음</p>
													</c:if>
													<c:if test="${meo.read != 'NO'}">
														<p class="messageread">읽음</p>
													</c:if>
												</div>
												<div> ${meo.title} </div>
												<div>
													<span class="underText">받는사람 : ${meo.receivename}(${meo.receiveempno}) |
