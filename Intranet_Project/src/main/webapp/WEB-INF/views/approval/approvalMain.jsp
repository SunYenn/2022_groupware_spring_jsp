		...
		<div class="con_middle">
			...
			<div class="content">
				...
				<div class="approval_main_head">
					...
					<div>
						<div>
							<h4>결재반려 : ${countCancel}건</h4>
						</div>
						<div>
							<button onclick="location.href='${path}/approval/approvalList?approvalStatus=결재반려&userNo=${EmpVO.empno}'">결재반려</button>
						</div>
					</div>
				...
				<table class="approval_main_body">
					<c:if test="${empty mainList}">
						<tr><td colspan="7">조회된 결재목록이 없습니다.</td></tr>
					</c:if>
					...
					<c:if test="${mainList != null}">
						<c:forEach var="list" items="${mainList}">
							...
							<tr onclick="location.href='${appmove}'" class="pointer">
								<td>${list.rowNum}</td>
								<td align="center">${list.appKinds}</td>
								<td>${apptitle}</td>
								<td>${list.userName}</td>
								<td>${deptName}</td>
