...
public class main {
	...
	public String Manager_Meun(HttpServletRequest request, Model model) {
		...
		GetList waitingList = new GetList(PageSize, totalCount, currentPage);		
		HashMap<String, Integer> hmap = new HashMap<String, Integer>();
		hmap.put("startNo", waitingList.getStartNo());
		hmap.put("endNo", waitingList.getEndNo());		
		waitingList.setEmpList((ArrayList<EmpVO>) mapper.selectwaiting(hmap));
		model.addAttribute("waitingList", waitingList);
		
		// 결재 대기 목록
		GetList paging = new GetList();
	...
	public String emp_list(HttpServletRequest request, Model model) {
		...
		if (searchdeptno == null || searchdeptno.trim().length() == 0 && searchname == null) {
			...
			empList.setEmpList((ArrayList<EmpVO>) mapper.selectemp(hmap));
			...
			empList.setEmpList((ArrayList<EmpVO>) mapper.selectByName(param));			
		} else { // 팀, 이름 같이 조회
			Param param = new Param();
	...
	public String show_all_board(HttpServletRequest request, Model model) {
		...
		if (category == null && searchobj == null) {
			...
			boardlist.setBoardList(mapper.AllBoardSelect(param));
			...
			boardlist.setBoardList(mapper.NoCatSelectMulti(param));			
		} else {
			param.setCategory(category);
			param.setSearchobj(searchobj);
			param.setSearchcategory(searchcategory);
...
//		=============쿠키 ==============
		...
		GetList commentlist = new GetList(PageSize, totalCount, currentPage);
	...
	public String attend_list(HttpServletRequest request, Model model) {
		...
		if (searchdeptno == null && searchdate == null && searchname == null) {
			...
			attendList.setAttList((ArrayList<AttendVO>) mapper.selectAttendAll(hmap));
			...
			attendList.setAttList((ArrayList<AttendVO>) mapper.selectAttendByDept(hmap));
			...
			attendList.setAttList((ArrayList<AttendVO>) mapper.selectAttendByDate(param));
			...
			attendList.setAttList((ArrayList<AttendVO>) mapper.selectAttendByDeptDate(param));
	...
	@RequestMapping("/move_perform_1")
	public String move_perform_1(HttpServletRequest request, Model model) {
		...
		return "redirect:perform_1";
	}
	
	@RequestMapping("/perform_1")
	public String perform_1(HttpServletRequest request, Model model) {
